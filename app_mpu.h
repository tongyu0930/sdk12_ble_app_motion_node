...
typedef struct
{
#if defined(MPU9250) || defined(MPU60x0)
    uint8_t                 :3;
#else
 ...
 *
 * THIS FUNCTION DOES NOT WORK ON MPU60x0 and MPU9250 AS THEY DON'T HAVE THE
 ...
 * MPU9150 has an AK8975C and MPU9250 an AK8963 internal magnetometer. Their register maps
 * are similar, but AK8963 has adjustable resoultion (14 and 16 bits) while AK8975C has 13 bit resolution fixed. 
 */


#if (defined(MPU9150) || defined(MPU9250)) && (TWI_COUNT >= 1) // Magnetometer only works with TWI so check if TWI is enabled
...
/**@brief Enum defining possible output bit resolutions for MPU9250 */
#if defined(MPU9250)
 ...
 * (and bit resolution for MPU9250).
 */
typedef struct
{
	uint8_t mode : 4;  
