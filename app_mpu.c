...
uint32_t mpu_init(void)
{
    ...
    err_code = nrf_drv_mpu_write_single_register(MPU_REG_SIGNAL_PATH_RESET, reset_value); //这一步有问题！！！
    if(err_code != NRF_SUCCESS) return err_code;
...
// Function does not work on MPU60x0 and MPU9250
#if defined(MPU9150)
...
#endif // defined(MPU9150) || defined(MPU9250) && (TWI_COUNT >= 1)
